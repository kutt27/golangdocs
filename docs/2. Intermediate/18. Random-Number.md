...
!!! abstract "Overview"
    Master Go's random number generation capabilities using math/rand and crypto/rand packages. Learn pseudorandom and cryptographically secure random number generation, seeding, range generation, and applications for games, simulations, and security-sensitive systems.

!!! tip "Key Points"
    - Use math/rand for simulations, games, and non-cryptographic purposes
    - Use crypto/rand for security-sensitive applications like tokens and keys
    - Always seed the random generator with rand.Seed() for different sequences
    - Generate random numbers in ranges with proper boundary handling
    - Implement shuffling, sampling, and weighted selection algorithms
    - Essential for games, simulations, cryptography, and statistical sampling
...
!!! info "Random Number Components"
    ```mermaid
    graph LR
        A[Random Numbers] --> B[math/rand]
        A --> C[crypto/rand]
        B --> D[Pseudorandom]
        B --> E[Fast]
        B --> F[Non-cryptographic]
        C --> G[Secure]
        C --> H[Slower]
        C --> I[Cryptographic]
...
!!! example "Simple Random Numbers"
    ```go title="simple_random.go" linenums="1" hl_lines="8-18"
    package main
...
!!! example "Seeding the Random Number Generator"
    ```go title="seeding.go" linenums="1" hl_lines="8-22"
    package main
...
!!! example "Integer Range"
    ```go title="integer_range.go" linenums="1" hl_lines="8-22"
    package main
...
!!! example "Float Range"
    ```go title="float_range.go" linenums="1" hl_lines="8-22"
    package main
...
!!! example "Random Booleans"
    ```go title="random_booleans.go" linenums="1" hl_lines="8-20"
    package main
...
!!! example "Random Strings"
    ```go title="random_strings.go" linenums="1" hl_lines="8-24"
    package main
...
!!! example "Random Bytes"
    ```go title="random_bytes.go" linenums="1" hl_lines="8-20"
    package main
...
!!! example "Using crypto/rand"
    ```go title="crypto_rand.go" linenums="1" hl_lines="8-24"
    package main
...
!!! example "Secure Random String Generation"
    ```go title="secure_random_string.go" linenums="1" hl_lines="8-30"
    package main
...
!!! example "Shuffling a Slice"
    ```go title="shuffling_slice.go" linenums="1" hl_lines="8-24"
    package main
...
!!! example "Random Sampling"
    ```go title="random_sampling.go" linenums="1" hl_lines="8-32"
    package main
...
!!! example "Weighted Random Selection"
    ```go title="weighted_random.go" linenums="1" hl_lines="8-40"
    package main
...
!!! example "Normal Distribution"
    ```go title="normal_distribution.go" linenums="1" hl_lines="8-26"
    package main
...
!!! example "Exponential Distribution"
    ```go title="exponential_distribution.go" linenums="1" hl_lines="8-24"
    package main
...
!!! example "Dice Game Simulator"
    ```go title="dice_game.go" linenums="1" hl_lines="8-60"
    package main
...
!!! tip "Seeding the Random Generator"
    Always seed the random generator to ensure different sequences on each run.

!!! example "Seeding the Random Generator"
    ```go title="seeding_practice.go" linenums="1" hl_lines="8-16"
    package main
...
!!! tip "Using crypto/rand for Security"
    Use crypto/rand for security-sensitive applications like tokens and keys.

!!! example "Using crypto/rand for Security"
    ```go title="crypto_practice.go" linenums="1" hl_lines="8-20"
    package main
...
!!! tip "Handling Range Correctly"
    Be careful with range boundaries to avoid off-by-one errors.

!!! example "Handling Range Correctly"
    ```go title="range_practice.go" linenums="1" hl_lines="8-18"
    package main
...
!!! tip "Reusing Random Sources"
    Create and reuse random sources for better performance.

!!! example "Reusing Random Sources"
    ```go title="reuse_sources.go" linenums="1" hl_lines="8-20"
    package main
...
!!! warning "Bulk Random Data"
    Use rand.Read for generating large amounts of random data efficiently.

!!! example "Bulk Random Data"
    ```go title="bulk_random.go" linenums="1" hl_lines="8-22"
    package main
...
!!! warning "Pre-allocating for Random Strings"
    Pre-allocate slices when generating random strings for better performance.

!!! example "Pre-allocating for Random Strings"
    ```go title="preallocate_strings.go" linenums="1" hl_lines="8-26"
    package main
    ...
    func main() {
        rand.Seed(time.Now().UnixNano())
        fmt.Println("Random string:", randomString(16))
    }
    ```
...
!!! success "Key Takeaways"
    - **Basic Functions**: Use `rand.Intn()` for integers, `rand.Float64()` for floats
    - **Seeding**: Always seed with `rand.Seed(time.Now().UnixNano())` for different sequences
    - **Range Generation**: Use `min + rand.Intn(max-min+1)` for inclusive ranges
    - **Security**: Use `crypto/rand` for tokens, keys, and passwords
    - **Shuffling**: Use `rand.Shuffle()` for randomizing slices
    - **Performance**: Pre-allocate slices and reuse random sources

!!! quote "Remember"
    "Random number generation is a fundamental tool in Go programming with applications across many domains. By understanding both math/rand and crypto/rand, you can implement everything from simple games to complex cryptographic systems. Always choose the right package for your needs and follow best practices to ensure your random number generation is both efficient and secure."}